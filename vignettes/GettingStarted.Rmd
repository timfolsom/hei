...
To get started, first we load the hei package and the [dplyr](http://dplyr.tidyverse.org/) package. The latter, in case you are unfamiliar, provides access to a number of useful functions that, while not necessary, greatly simplify some of the data manipulation steps and as such is recommended and will be used throughout these examples. Some of the functions included are filter(), group_by(), and select(), as well as the ``` %>% ``` "pipe."
