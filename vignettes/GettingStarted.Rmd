...
## Sealing the deal with the combo() and hei() fucntions

The combo() function takes as its arguments three data sets: an FPED data set, a NHANES dietary data set, and an NHANES demographic data set. In other words, combo() simply takes as its arguments the outputs of the first three functions and outputs a single data set containing all the variables necessary to calculate HEI scores from as well as key demographic data. The hei() function, the culmination of this process, takes a dietary data set and outputs a data set with an additional column containing an HEI score for each participant.
...
hei0910 <- hei(diet0910,demo0910,fped0910)
```
